use crate::{
    calc_pairing_precomp::{self, PairingPrecompStark},
    ecc_aggregate::{self, ECCAggStark},
    final_exponentiate::{self, FinalExponentiateStark},
    fp12_mul::{self, FP12MulStark},
    fp_plonky2::N,
    g1_plonky2::{pk_point_check, PUB_KEY_LEN},
    g2_plonky2::{signature_point_check, SIG_LEN},
    hash_to_curve::hash_to_curve,
    miller_loop::{self, MillerLoopStark},
    native::{self, Fp, Fp12, Fp2},
};
...
use starky::{proof::StarkProofWithPublicInputsTarget, util::trace_rows_to_poly_values};
...
pub fn fp12_mul_main<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    x: Fp12,
    y: Fp12,
) -> (
    FP12MulStark<F, D>,
...
fn ec_aggregate_main<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    points: Vec<[Fp; 2]>,
    res: [Fp; 2],
    bits: Vec<bool>,
) -> (
...
fn generate_aggregate_proof(
    ...
    let points: Vec<[Fp; 2]> = pub_keys
        ...
        .map(|i| {
            ...
            [
                Fp::get_fp_from_biguint(BigUint::from_bytes_be(
                    &hex::decode(res.point.getx().to_string()).unwrap(),
                )),
                Fp::get_fp_from_biguint(BigUint::from_bytes_be(
                    &hex::decode(res.point.gety().to_string()).unwrap(),
                )),
            ]
        })
        .collect::<Vec<[Fp; 2]>>();
    ...
    for i in 0..pub_keys.len() {
        if bits[i] == true {
            public_keys.push(
                PublicKey::from_bytes(&hex::decode(pub_keys[i][3..99].to_string()).unwrap())
                    .unwrap(),
            );
        }
    }
    ...
    let apk =
        AggregatePublicKey::aggregate(&(&public_keys.iter().collect::<Vec<&PublicKey>>())).unwrap();
    let res: [Fp; 2] = [
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(apk.point.getx().to_string()).unwrap(),
        )),
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(apk.point.gety().to_string()).unwrap(),
        )),
    ];
    println!("ec aggregate stark");
    let (stark_ec, proof_ec, config_ec) = ec_aggregate_main::<F, C, D>(points, res, bits.clone());
    let recursive_ec =
        recursive_proof::<F, C, ECAggStark, C, D>(stark_ec, proof_ec, &config_ec, true);
    ...
    let q_x1 = Fp2([
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&result.getx().geta().to_string()).unwrap(),
        )),
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&result.getx().getb().to_string()).unwrap(),
        )),
    ]);
    let q_y1 = Fp2([
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&result.gety().geta().to_string()).unwrap(),
        )),
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&result.gety().getb().to_string()).unwrap(),
        )),
    ]);
    let q_z1 = Fp2([
    ...
    let (stark_pp1, proof_pp1, config_pp1) = calc_pairing_precomp::<F, C, D>(q_x1, q_y1, q_z1);
    let recursive_pp1 =
        recursive_proof::<F, C, PpStark, C, D>(stark_pp1, proof_pp1.clone(), &config_pp1, true);

    println!("miller_loop stark 1");
    let (stark_ml1, proof_ml1, config_ml1) =
    ...
    let q_x2 = Fp2([
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&signature_points.point.getx().geta().to_string()).unwrap(),
        )),
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&signature_points.point.getx().getb().to_string()).unwrap(),
        )),
    ]);
    let q_y2 = Fp2([
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&signature_points.point.gety().geta().to_string()).unwrap(),
        )),
        Fp::get_fp_from_biguint(BigUint::from_bytes_be(
            &hex::decode(&signature_points.point.gety().getb().to_string()).unwrap(),
        )),
    ]);
    let q_z2 = Fp2([
        Fp::get_fp_from_biguint(BigUint::from_str("1").unwrap()),
    ...
    let (stark_pp2, proof_pp2, config_pp2) = calc_pairing_precomp::<F, C, D>(q_x2, q_y2, q_z2);
    let recursive_pp2 =
        recursive_proof::<F, C, PpStark, C, D>(stark_pp2, proof_pp2.clone(), &config_pp2, true);

    println!("miller_loop stark 2");
    let (stark_ml2, proof_ml2, config_ml2) =
    ...
    let recursive_fp12_mul = recursive_proof::<F, C, Fp12MulStark, C, D>(
        stark_fp12_mul,
        proof_fp12_mul.clone(),
        &config_fp12_mul,
        true,
    );
    ...
    let pks = prev_sync_committee_update
        .next_sync_committee
        .pubkeys
        .0
        .iter()
    ...
    (proof, verifier_only, common_data)
}
...
pub fn aggregate_proof(
    ...
    let proof_tuple = std::thread::Builder::new()
        .spawn(move || {
            return generate_aggregate_proof(
                pub_keys,
                sync_aggregate,
                signing_root,
                prev_sync_committee_update,
            );
        })
        .unwrap()
        .join()
    ...
    InnerC::Hasher: plonky2::plonk::config::AlgebraicHasher<F>,
{
    ...
    let pt = starky::recursive_verifier::add_virtual_stark_proof_with_pis(
        ...
        inner_config,
        degree_bits,
        // TODO: this should be inspected cause we passed zeros just to satisfy the interface
        0,
        0,
    );
    ...
    InnerC::Hasher: plonky2::plonk::config::AlgebraicHasher<F>,
{
    ...
    let pt = define_recursive_proof::<F, C, S, InnerC, D>(
        stark,
        &inner_proof,
        inner_config,
        print_gate_counts,
    ...
    InnerC::Hasher: AlgebraicHasher<F>,
{
    ...
    for idx in 0..pk_targets.len() {
        let pk_point_x = BigUintTarget {
            limbs: (0..N)
                .into_iter()
                .map(|i| U32Target(pt_ec.public_inputs[ecc_aggregate::POINTS + idx * 24 + i]))
                .collect(),
        };
        let pk_point_y = BigUintTarget {
            limbs: (0..N)
                .into_iter()
                .map(|i| U32Target(pt_ec.public_inputs[ecc_aggregate::POINTS + idx * 24 + 12 + i]))
                .collect(),
        };
        builder.connect(
            pt_ec.public_inputs[ecc_aggregate::BITS + idx],
            bit_targets[idx].target,
        );
    ...
    for i in 0..12 {
        builder.connect(
            pt_ec.public_inputs[ecc_aggregate::RES + i],
            pt_ml1.public_inputs[miller_loop::PIS_PX_OFFSET + i],
        );
    }
    for i in 0..12 {
        builder.connect(
            pt_ec.public_inputs[ecc_aggregate::RES + i + 12],
    ...
    for i in 0..68 * 3 * 24 {
        builder.connect(
            pt_pp2.public_inputs[calc_pairing_precomp::ELL_COEFFS_PUBLIC_INPUTS_OFFSET + i],
            pt_ml2.public_inputs[miller_loop::PIS_ELL_COEFFS_OFFSET + i],
        );
    }
    ...
    for i in 0..N {
        builder.connect(
            pt_ml2.public_inputs[miller_loop::PIS_PX_OFFSET + i],
            neg_generator_x.limbs[i].0,
        );
        builder.connect(
            pt_ml2.public_inputs[miller_loop::PIS_PY_OFFSET + i],
            neg_generator_y.limbs[i].0,
        );
    }
    ...
    let inner_data_pp1 =
        builder.add_virtual_verifier_data(inner_cd_pp1.config.fri_config.cap_height);
    let inner_data_ml1 =
        builder.add_virtual_verifier_data(inner_cd_ml1.config.fri_config.cap_height);
    let inner_data_pp2 =
        builder.add_virtual_verifier_data(inner_cd_pp2.config.fri_config.cap_height);
    let inner_data_ml2 =
        builder.add_virtual_verifier_data(inner_cd_ml2.config.fri_config.cap_height);
    let inner_data_fp12m =
        builder.add_virtual_verifier_data(inner_cd_fp12m.config.fri_config.cap_height);
